...
// CALCULATE ARB //////////////////////////
async function calculateArbitrage(amountIn, amountOut, surfaceObj) {
  ...
  if (profitLoss >= threshold) {
    ...
    // await action(fileJsonArrayLimit, index - 1, amountIn * 10)
    // console.log(resultArray)
  }
...
async function getPrice(factory, amtIn, tradeDirection) {
  ...
  // Get Provider
  const provider = new ethers.providers.JsonRpcProvider("https://mainnet.infura.io/v3/03584ad9e8a549d3ad62026e07c3444f");
  const ABI = [
...
async function getDepth(amountIn) {
  ...
  while (true) {
    ...
    let fileInfo = getFile("C:/Users/Wajah/PycharmProjects/UniSwapV3/uniswap_surface_rates.json");
    fileJsonArray = JSON.parse(fileInfo);
    let limit = fileJsonArray.length
    fileJsonArrayLimit = fileJsonArray.slice(0, limit);
    console.log(fileJsonArray.length)
    // Loop through each trade and get Price information
    for (let index = 0; index < fileJsonArrayLimit.length; index++) {
      await action(fileJsonArrayLimit, index, amountIn)
...
async function action(fileJsonArrayLimit, i, amountIn) {
